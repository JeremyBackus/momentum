...
momentum:
	...
	RI_RegisterDevices()	;RawInput register. Flag QS_RAWINPUT = 0x0400
	...
	TrayTip, momentum, Enabled, 0, 0
velocity_monitor:
Loop {		;Velocity Loop - pointer movement monitor.
	Sleep, -1
	DllCall("Sleep", "UInt", 19)
	If !((DllCall("GetQueueStatus", "UInt", 0x0400) >> 16) & 0xFFFF) {
		If (Array2+Array1+Array0<1700) { ;Compare filtered average speed to Glide Activation Threshold.
			i:=mod(A_Index,3), Array%i%:=0.0	;Update speed reading.
	...
	;Calculate: pointer velocity vx/vy, pointer speed v. Store speed and velocity components for each data point.
	DllCall("QueryPerformanceCounter", "Int64*", cT1), DllCall("GetCursorPos", "Ptr", lpP_addr), i:=mod(A_Index,3), ArrayX%i%:=vx:=2531252*((x1:= NumGet(lpP, 0, "Int"))-x0)/(cT1-cT0), ArrayY%i%:=vy:=2531252*((y1:= NumGet(lpP, 4, "Int"))-y0)/(cT1-cT0), Array%i%:=Sqrt(vx**2+vy**2), x0:=x1, y0:=y1, cT0:=cT1
	}
;Get highest speed and equivalent velocity readings within moving average window.
	v := Array0>Array1 ? (Array0>Array2 ? [0.665*ArrayX0,0.665*ArrayY0] : [0.665*ArrayX2,0.665*ArrayY2]) : (Array1>Array2 ? [0.665*ArrayX1,0.665*ArrayY1] : [0.665*ArrayX2,0.665*ArrayY2]), Array2:=Array1:=Array0:=0.0
Loop {		;Gliding Loop - pointer glide.
	;Calculate elapsed time from Velocity Loop exit and simulate inertial pointer displacement.
	DllCall("QueryPerformanceCounter", "Int64*", cT0), fff:= 1-Exp((cT0-cT1)*-0.593/1000000), DllCall("SetCursorPos", "int", v[1]*fff+x1, "int", v[2]*fff+y1), DllCall("Sleep", "UInt", 1)
...
VarInit:	;Initialise variables.
	...
	i:=0, vx:=vy:=0.0, v:=[], v:=[0, 0], fff:=0.0
	Array0:=Array1:=Array2:=0.0	;Speeds
...
if (!FileExist(iniFile) OR resetDefaults=="ERROR")	{
    MsgBox, 67,Welcome to momentum ver. %ver% by IOrot,It is highly recommended to use the default Windows settings for your pointing device.`nProceed changing to defaults at launch?`n(Revert anytime at 'Start>Mouse>Pointer Options>Motion').
	IfMsgBox Yes
...
OSPointerSettings: ;Restore to default any OS pointer settings and confirm OS pointer settings values.
	...
	If NOT(skipStartupDialog) {
		MsgBox, 3,  momentum %ver% by IOrot, Speed Glide Threshold:`t%st%`n`n   Windows Mouse Parameters`n`tSpeed:`t%MouseSpeed%`n`tAcceleration`n`t  EnhPPr:`t%acOn%`n`t  Thr1:`t%acThr1%`n`t  Thr2:`t%acThr2%`n`n%resetText%`n`nRetain momentum launch options?, 7
	IfMsgBox No
		{
